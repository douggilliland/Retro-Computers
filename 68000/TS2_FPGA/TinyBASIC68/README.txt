...
32  bits  so  that the PEEË and POKE commands  could  access  the  
...
There are 2¶ variables denoted by the letters A through Z.  There  
...
Wilì  set  the variable A to the value of the expression  234-5*¶  
(i.e.  204),  set  the  variable A (again© to the  value  of  the  
Šexpression A/2 (i.e. 102), set the variable Ø to the value of the  
...
Wilì print the values of A and M in 1± spaces,  the values of  C,  
...
the answer to a yes-or-no question, such as
     ...
     10 Y=999¹ º INPUT 'Are you sleepy?'A º IÆ A=Ù GOTÏ 100

This  works  because the user can answer the  question  with  the  
expression  'Y',  which  puts the numeric value of Ù into  the  A  
variable.


     INPUT 'What is the weight'A, "and size"B

This  is  the  same as the first INPUT example  except  that  the  Šprompt  
...
and  "PRIN.¢  alì  stand  for "PRINT".   The word  "LET¢  in  LET  
...
AUXIÎ  and AUXOUT subroutines to use the cassette  interface.  (I  
...
only modify the SAVE and LOAD subroutines.
     One  warningº  the  DIRECT  and EXEÃ routines  were  written  
...
following  formsº   8-inch CP/M SSSD diskette,  5-inch Osborne or  
...
          of BASIC "LET A=B=0¢ means "set both A and M to 0".  To  
          this  version  of Tiny BASIC,  it means "set A  to  the  Š          result of comparing M with 0".
...
In line 10,  A wilì be set to 12³ if X>Y,  to 45¶ if X=Y,  and to  
78¹ if X<Y. In line 20, the "*¢ operator acts like a logicaì AND,  
and the "+¢ operator acts like a logicaì OR.  In line 30,  A wilì  
...
A  Tiny BASIC line consists of a line number between ± and  6553´  
...
Wilì  set  the variable A to the value of the expression  234-5*¶  
(i.e.  204),  set  the  variable A (again© to the  value  of  the  
expression A/2 (i.e. 102), set the variable Ø to the value of the  
expression A-100 (i.e. 2), and then set the variable @(11© to 10±  
(where 1± is the value of the expression X+¹ and 10± is the value  
of the expression A-1).
...
Wilì  set the variable Õ to either ± or 0 depending on whether  A  
is  not  equaì  to or is equaì to B»  and set the variable  Ö  to  
either  X,  Ù or 0 depending on whether A is greater  than,  less  
than, or equaì to B.
...
ŠWilì  print the values of A and M in 1± spaces,  the values of C,  
D,  and E in ³ spaces, and the values of Æ and Ç in 10 spaces. If  
there  aren't  enough  spaces specified for a given value  to  be  
printed, the value wilì be printed in fulì anyway.
...
A  wilì  be set to the value of this  expression,  then  "B:¢  is  
...
A  RETURÎ command must be the last command on a line and must  be  
...
remembered.  Execution then continues the normaì way untiì a NEXT  
...
alì stand for "PRINT".  The word LET in LET commands may also  be  
