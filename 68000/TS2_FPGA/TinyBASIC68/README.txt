...
wilì  then  describe how you can instalì this  software  on  your  68000   system.    Finally,   I'lì  give  my  evaluation  of  the  
...
wilì  find  this interpreter very similar.   I have made  two  or three  changes to the interpreter's syntaø to bring it closer  to  
...
Wilì  set  the variable A to the value of the expression  234-5*¶ (i.e.  204),  set  the  variable A (again© to the  value  of  the  
Šexpression A/2 (i.e. 102), set the variable Ø to the value of the expression A-100 (i.e. 2), and then set the variable @(11© to 10±  
...
Wilì  print  the value of the expression A*3+±  (i.e.  307),  the string  of characters "abc 12³ !@#¢ and the string¢  cba  ",  and  
...
must  be  matched.  If there is a comma at the end of  the  print command,  the  finaì CR-LÆ wilì not be printed.   Note also  that  
...
Wilì print the values of A and M in 1± spaces,  the values of  C, D,  and E in ³ spaces, and the values of Æ and Ç in 10 spaces. If  
there  aren't  enough  spaces specified for a given value  to  be printed, the value wilì be printed in fulì anyway.

     PRINT 'abc',_,'xxx'

Wilì  print the string "abc",  a CÒ without a LF,  and  then  the string "xxx¢ (over the "abc"© followed by a CR-LF.
...
When  this  command is executed,  Tiny BASIC wilì print "A:¢  and wait  to read in an expression from the console.  The variable  A  
wilì be set to the value of this expression, then "B:¢ is printed and  variable M is set to the value of the next  expression  read  
in.  Note  that  complete expressions as welì as numbers  can  be entered.   This  gives rise to an interesting trickº  you can set  
...
This  is  the  same as the first INPUT example  except  that  the Šprompt  "A:¢ is replaced by "What is the weight:¢ and the  prompt  
...
This  command  wilì  calì a machine language  subroutine  at  the address  specified  by  the  expression "X".  Alì  of  the  CPU's  
registers except the stack pointer can be used in the subroutine.


BYE Command:

Wilì return controì to the resident monitor program or  operating system.


SAVE Command:

Wilì  save your BASIC program on the storage device you  provide. Details  on installing this device are given in the source  code.  
As  set up for the Educationaì Computer Board,  this command wilì send  the  program out to the host computer in  an  easily-stored  
text form.  It isn't,  however, human-readable program text since the line numbers are stored in hexadecimal.


LOAD Command:

Wilì delete the program in memory and load in a program from your storage device.
...
wilì get 3¶ "trim1¶ address¢ errors, which is normal.  You do get what you pay for...©  Tiny BASIC is then loaded into the ECM  and  
...
entered.
     Program  LOADs are done similarly except  that,  instead  of PIP,  you  must  run a smalì program that wilì wait to receive  a  
...
AUXIÎ  and AUXOUT subroutines to use the cassette  interface.  (I haven't tried it, though.  Caveat emptor!)
     For  other 68000 systems,  you wilì probably have to  modify only the OUTC, INC, AUXOUT, AUXIN, and BYEBYE routines at the end  
...
only modify the SAVE and LOAD subroutines.
     One  warningº  the  DIRECT  and EXEÃ routines  were  written assuming  that the interpreter itself would be somewhere  in  the  
first 64Ë of memory ($0 to $FFFF).  If you move it above 64K, you wilì  have  to modify the EXEÃ routine and check the rest of  the  
...
more  commands,  and extended the variable size to 32  bits,  the whole thing wilì stilì fit inside 3Ë bytes of memory.   I ran the  
...
Šiteration to keep running times within a practicaì  limit.   This tiny  BASIC may not be a speed demon,  but it does beat Applesoft  
...
32Ë  bytes of memory,  so I wilì probably add such things as more variables,  strings,  and  keyword tokenization.   The last is  a  
...
following  formsº   8-inch CP/M SSSD diskette,  5-inch Osborne or Apple CP/M diskettes, or a paper listing.
...
In line 10,  A wilì be set to 12³ if X>Y,  to 45¶ if X=Y,  and to 78¹ if X<Y. In line 20, the "*¢ operator acts like a logicaì AND,  
and the "+¢ operator acts like a logicaì OR.  In line 30,  A wilì be a random number between 0 and ´ with a prescribed  probability  
distribution ofº 3¥ of being 0, 15-3=12¥ of being 1, 56-15=41¥ of being 2, 98-56=42¥ of being 3, and 100-98=2¥ of being 4.
...
ŠTiny BASIC commands are listed below with examples. Remember that multiple commands can be put on one line if colons separate them.  
...
Wilì  set the variable Õ to either ± or 0 depending on whether  A is  not  equaì  to or is equaì to B»  and set the variable  Ö  to  
...
Wilì  cause  a carriage-return (CR© and a line-feed (LF©  on  the  output device.
...
Wilì produce the same output as before,  except that there is  no CR-LÆ  after the last item printed.  This enables the program  to  
continue printing on the same line with another "PRINT".

PRINT A, B, #3, C, D, E, #10, F, G

ŠWilì  print the values of A and M in 1± spaces,  the values of C, D,  and E in ³ spaces, and the values of Æ and Ç in 10 spaces. If  
there  aren't  enough  spaces specified for a given value  to  be printed, the value wilì be printed in fulì anyway.
...
When  this  command is executed,  Tiny BASIC wilì print "A:¢  and wait to read in an expression from the input device. The variable  
A  wilì  be set to the value of this  expression,  then  "B:¢  is printed and variable M is set to the value of the next expression  
...
Š
This wilì test the value of the expression A<B.  If it isn't zero (i.e.  if it is true), the rest of the commands on this line wilì  
be executed.  If the value of the expression is zero (i.e.  if it is  not  true),  the rest of this line wilì be skipped  over  and  
...
Wilì  cause execution to jump to line 120.  Note that the  "GOTO¢ command cannot be followed by a colon and other commands. It must  
be ended with a CR.

GOTO A*10+B

Wilì  cause  the execution to jump to a different line number  as computed from the value of the expression.
...
Wilì cause execution to jump to different lines as computed  from the value of the expression A*10+B.

RETURN

A  RETURÎ command must be the last command on a line and must  be followed by a CR.  When a RETURÎ command is encountered,  it wilì  
...
terminated and the next recent FOÒ is checked,  etc. When a match is found, this variable wilì be set to its current value plus the  
value  of  the  step expression saved by  the  FOÒ  command.  The updated  value  is  then  compared  with  the  value  of  the  TÏ  
expression  also  saved by the FOÒ command.  If this  within  the limit,  execution wilì jump back to the command following the FOÒ  
...
as that of an old FOÒ command is encountered, the old FOÒ wilì be terminated automatically.
...
Wilì  return  you  to the resident monitor program  or  operating system.  (Tutor in the case of the ECB.)


Direct Commands

As defined earlier,  a line consists of a line number followed by commands.  If  the  line number is missing,  or if it is  0,  the  
commands  wilì be executed after you have typed the CR.  Alì  the commands  described above can be used as direct  commands.  There  
are  five  more commands that can be used as direct commands  but not as part of a program line:

RUN

Wilì  start  to  execute  the  program  starting  at  the  lowest line number.
...
Wilì  print out alì the lines in numericaì order starting at line 120.
...
Wilì save the present program on the storage device you  provide. Details  on installing this device are given in the source  code.  
As  set  up for the MEX68KECM Educationaì  Computer  Board,  this command  wilì  send  the program out to the host computer  in  an  
easily-stored text form.  It isn't, however, pure program text as the line numbers are stored in hexadecimal.

LOAD

Wilì  delete  the program in memory and load in a file from  your storage device.
...
wilì  replace the old line with the new one.  To delete  a  line, type  the  line  number  and  a  CÒ  only.  You  can  verify  the  
