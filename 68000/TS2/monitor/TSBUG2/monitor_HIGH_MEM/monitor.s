...
*
         .org     0xf8000000        | Monitor origin
         .long    STACK             | Reset stack pointer
         RESET_V = 0x00008008
         .long    RESET_V             | Reset vector
...
LOAD:    MOVE.L   CN_OVEC(%A6),-(%A7) | Save current output device name
         ...
         DCB4_V = 0x00008c22            | Works around GNU assembler issue
         MOVE.L   #DCB4_V,CN_OVEC(%A6)  | Set up aux ACIA as output
         DCB3_V = 0x00008c10            | Works around GNU assembler issue
         MOVE.L   #DCB3_V,CN_IVEC(%A6)  | Set up aux ACIA as input
...
ST_DCB2: MOVE.B  (%A1)+,(%A0)+     | Move the 16 bytes of a DCB header
         ...
         DCB1_V = 0x00008bec         | Works around GNU assembler issue
         MOVE.L  #DCB1_V,CN_IVEC(%A6) | Set up vector to console input DCB
         DCB2_V = 0x00008bfe         | Works around GNU assembler issue
         MOVE.L  #DCB2_V,CN_OVEC(%A6) | Set up vector to console output DCB
...
X_SET:  LEA.L   X_BASE,%A0         | Point to base of exception table
        ...
        X_UN_V = 0x000089e4          | Works around GNU assembler issue
X_SET1: MOVE.L  #X_UN_V,(%A0)+       | Store uninitialized exception vector
        DBRA    %D0,X_SET1         | Repeat until all entries preset
        SUB.L   %A0,%A0            | Clear A0 (points to vector table)
        BUS_ER_V = 0x000087b4        | Works around GNU assembler issue
        MOVE.L  #BUS_ER_V,8(%A0)     | Setup bus error vector
        ADD_ER_V = 0x000087c2        | Works around GNU assembler issue
        MOVE.L  #ADD_ER_V,12(%A0)    | Setup address error vector
        IL_ER_V = 0x0000879e         | Works around GNU assembler issue
        MOVE.L  #IL_ER_V,16(%A0)     | Setup illegal instruction error vect
        TRACE_V = 0x00008898         | Works around GNU assembler issue
        MOVE.L  #TRACE_V,36(%A0)     | Setup trace exception vector
        TRAP_0_V = 0x00008652        | Works around GNU assembler issue
        MOVE.L  #TRAP_0_V,128(%A0)   | Setup TRAP #0 exception vector
        BRKPT_V = 0x000087d0         | Works around GNU assembler issue
        MOVE.L  #BRKPT_V,184(%A0)    | Setup TRAP #14 vector = breakpoint
        AWARM_V = 0x00008040         | Works around GNU assembler issue
        MOVE.L  #AWARM_V,188(%A0)    | Setup TRAP #15 exception vector
