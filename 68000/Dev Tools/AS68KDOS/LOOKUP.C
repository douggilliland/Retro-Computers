...
install(char *str, long val, char type)
{
	...
	if( (np = lookup(str)) != NULL )
	{
		...
		if( np->sym_type != SYM && np->sym_type != SXREF)
...
if(Debug&LOOKUP)printf("Install: %s=0x%lx(%s)\n",str,val,tnames[type]);
...
/*
 *      print_syms --- print symbol table entries
 */
void  print_syms()
{
	int i, prt;
	...
	for(i=0;i<HASHSIZE;i++)
	{
		for(s = hashtab[i]; s != NULL; s=s->sym_next)
		{
			switch (s->sym_type)
			{
				case SYM:  strcpy(st, "SYM");
				           prt = 1;
							  break;

				case SXREF:  strcpy(st, "XREF");
				           prt = 1;
							  break;
