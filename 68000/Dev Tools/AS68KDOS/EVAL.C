 ...
 */
eval(long *result)
/* long *result    where to place result */
{
	long	left,right;	/* left and right terms for expression */
	int	ltype,rtype;
	...
	if(Debug&EVAL)printf("Eval Result=%lx(%s) %s\n",left,tnames[ltype],Force? "Fwdref":"");
	...
	*/
		...
		*tmp = '\0';
		if(*Optr == '\'')
			...
			error("Bad string");
		val = (long)strcpy(malloc(strlen(hold)+1),hold);
		minus = 0;
		ttype = QSTR;
	}
...
if(Debug&EVAL)printf("get_term <%s> result=%ld  ttype=%d\n",Optr, *result, ttype);
	return(ttype);
