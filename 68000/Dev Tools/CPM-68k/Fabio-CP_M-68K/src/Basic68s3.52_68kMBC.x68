...
*************************************************************************************
*                                                                                   *
*   Ver 3.52b - Changed for the 68k-MBC by J4F (2021)                               *
*                                                                                   *
*   HW ref: A091020, A091020-R140221 (or following HW revisions unless stated       *
*                                     otherwise)                                    *
*   REQUIRED: IOS S310121 (or following SW revisions unless stated otherwise)       *
*                                                                                   *
*   To find changes search the string "68k-MBC"                                     *
*                                                                                   *
...
*************************************************************************************
    ...
    INCLUDE "Basic68k3.41_68kMBC.inc"
...
; =========================================================================== ;
;
; Size the available RAM (128KB/512KB/1024KB) on the 68k-MBC
;
; Set into "found_ramSize" the size of RAM
;
...
start:
    ;
    ; Check for Lite/Full HW configuration (68k-MBC)
    ;
    ...
    ;
    move.b  #RAMCHK1,Addr1024           ; No, store RAMCHK1 into RAM
    ...
    bne     checkRam512                 ; No, jump and continue the check for 512KB (Z = 0)
    move.l  #RamSize1M, found_ramSize   ; Set the size for 1024KB RAM
    bra     code_start                  ; Jump to the start of Basic
...
checkRam512:
    ...
    cmp.b   #RAMCHK2,d0                 ; Equal to the original value?
    bne     found512                    ; No, jump
    bra     code_start                  ; Yes, 128KB RAM found. Jump to the start of Basic

found512:
    ;
    ; Found 512KB RAM
    ;
    move.l  #RamSize512K, found_ramSize ; Set the size for 512KB RAM
    bra     code_start                  ; Jump to the start of Basic
...
EXC_WR_OPCD EQU     IOBASE+0        ; Address of the EXECUTE WRITE OPCODE write port
EXC_RD_OPCD EQU     IOBASE+0        ; Address of the EXECUTE READ OPCODE read port
STO_OPCD    EQU     IOBASE+1        ; Address of the STORE OPCODE write port
...
; 68k-MBC Ram size equates
RAMCHK1     EQU     $55             ; RAM check test pattern 1
RAMCHK2     EQU     $AA             ; RAM check test pattern 2

VEC_OUT
*   MOVEM.l d0-d1,-(sp)             * save d0, d1
    move.b  #SERTX_OPC,(STO_OPCD).l ; Write SERIAL TX opcode to IOS
    move.b  d0,(EXC_WR_OPCD).l      ; Print current char
...
*   MOVE.l  d1,-(sp)                * save d1
    ...
    bne     RETCHR                  ; yes, jump and get it (Z = 0)
...
*   TST.b       d0                  * set the z flag
    ANDI.b  #$FE,CCR                ; No, clear the carry, flag we got no byte
*                                   * done by the TST.b
    RTS
...
*   TRAP        #15             * do I/O function - Not used for the 68k-MBC
...
*   TRAP        #15         * Not used for the 68k-MBC
...
*   MOVE.l  #ram_size,d0        * tell BASIC how big RAM is
    MOVE.l  found_ramSize,d0        * tell BASIC how big RAM is - Changed for the 68k-MBC
...
LAB_KESS
    ...
    CMP.w       #5,d0               * compare length with max+1
...
LAB_SMSG
    ...
    dc.b    'Enhanced 68k BASIC Version 3.52b - 68k-MBC',$0D,$0A,$0A,$00
...
found_ramSize   dc.l    RamSize128k         ; 68k-MBC RAM size (Default 128KB)
