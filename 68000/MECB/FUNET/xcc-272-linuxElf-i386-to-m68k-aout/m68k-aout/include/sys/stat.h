...
struct	stat 
{
  dev_t		st_dev;
#ifdef solaris
  long	st_pad[13];   /* <sys/stat.h> claims reserved for network id */
#endif
  ino_t		st_ino;
  mode_t	st_mode;
  short		st_nlink;
  ...
  dev_t		st_rdev;
#ifdef solaris
  long	st_pad2[2];   /* <sys/stat.h> offers no explanation */
#endif
  off_t		st_size;
  /* SysV/sco doesn't have the rest... */
#if defined(solaris)
  long		st_pad3[2]; /* <sys/stat.h> claims for future off_t expansion */
  timestruc_t   st_atim;
  timestruc_t   st_mtim;
  timestruc_t   st_ctim;
  long		st_blksize;
  long		st_blocks;
  char		st_fstype[ 16 ]; /* see <sys/stat.h> _ST_FSTYPSZ == 16 */
  long		st_pad4[8]; /* <sys/stat.h> claims for future expansion */
