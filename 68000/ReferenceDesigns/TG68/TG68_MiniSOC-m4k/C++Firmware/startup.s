...
l2
	...
	; initialise malloc before running global ctors

	move.l	#heap_low,d0
	addq	#7,d0
	and.l	#-8,d0	; Align to SDRAM burst boundary

	; FIXME - Derive RAM size from hardware registers
	move.l	#heap_top,d1
	sub.l	d0,d1
	move.l	d1,-(a7)
	move.l	d0,-(a7)
...
StandardPointer
	...
	; Start a new section to precede the constructors.
	section Init,code
run_init:
	; The ctors will just run one by one, and fall through into the next section...

	section	Fini,code
	; ... thus the next section begins with an rts.
	rts
run_fini:	; Academic; we're never likely to run dtors...

	Section Final,code
	; ... but for the sake of completeness we have an RTS here too.
	rts
