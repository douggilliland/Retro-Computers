...
/* int n   number of templates to check */
{
	...
	if( Size&p->sizes )
	{    /* quick test for MULTI/EMPTY/XREF templates */
		if(p->modes[0] == EMPTY )
			return(p);
		if(p->modes[0] == MULTI )
		{
...
/* long spec  specific ea for an operand */
{
...
scanea(struct ea *e)
{
   ...
   char  *pt, temp[MAXBUF];
	...
	e->reg = e->const1 = e->reg2 = e->const2 = 0;
	...
	case '#':       /* looks like immediate */
		...
		if (ttype == QSTR)   /* immediate char constant */
		{
			strcpy(temp, (char *)e->const1);      /* save string */
			free((void *)e->const1);              /* release buffer */

			/* move the last 4 bytes of the string to const1
				  in byte reversed order (80x86 memory order is
				  different from 680x0)  */
			e->const1 = 0L;
			pt = (char *)&e->const1;

			for (i = strlen(temp)-1, j = 0; i >= 0 && j < 4; i--, j++)
				pt[j] = temp[i];

			e->type = IMMED;

		}
		else if (ttype != SYM && ttype != SXREF)
	...
	case '\(':       /* could be almost anything */
		...
		case AREG:
			...
			else if( /*index*/strchr(",.*",*Optr) != NULL)
			{ /* possibly indexed */
				Optr = saveoptr; /* backup to An again */
				e->const1 = 0;	/* zero base displacement */
				do_indexed(e);
			}
			else
			{
		...
		case SYM:       /* could be EXPR,INDEX or PCINDEX */
			if( *Optr == '\)' )
			{
				...
				if (e->type == SXREF)
					...
					e->siz = optsize(e->const1);/* .[WL] allowed */
				e->type = EXPR;
				e->force = Force;
			}
			else if( *Optr == ',' )
		...
		case ZPC:
			Optr = saveoptr;
			e->const1 = 0;	/* supressed base register */
			do_indexed(e);
			break;
 ...
 *      with the value of SYM placed in e->const1.  The remaining
...
do_indexed(struct ea *e)
{
	...
	if( e->force || e->xn_sup || e->br_sup 
		|| e->const1 <MINBYTE || e->const1 >MAXBYTE){
		if( e->force || e->const1 <MINWORD || e->const1>MAXWORD)
...
if(Debug&XDEBUG){
	...
	if( idx[0].i_type == SYM || idx[0].i_type == SXREF ){  /* base displacement */
		e->bdsiz = (idx[0].i_siz==L ? 3 : 2);
		e->const1 = idx[0].i_val;
	...
	if( i>0 && (idx[i].i_type == SYM || idx[i].i_type == SXREF) ){ /* outer displacement */
...
/*
 *      do_xref --- scan XREF names and insert then into symbol table
 */
void do_xref()
{
  char  xname[33], sep=',';
  int  i;
	...
	while (sep == ',')
	{
		...
		install(xname, 0xEFFFFFFF, SXREF);
