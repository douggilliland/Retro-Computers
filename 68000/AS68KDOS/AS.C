...
long     Size;                   /* bitmap form of Ext                   */
long     Pc = 0;                 /* Program Counter                      */
long     Old_pc = 0;             /* Program Counter at beginning         */
...
char    **argv;
{
	...
	if( Err_count == 0 )
		print_syms();   /* print out symbol table */
...
doflag(char *s)
{
	...
	switch( *(s+1) ){
	case 'x':       sscanf(s+2, "%x", &Debug); break;
...
process()
{
	...
	{
		if( *Label && t->class != EQU && t->class != FEQU && t->class != IDNT)
			install(Label,Pc,SYM);
		if (t->class != XREF)
