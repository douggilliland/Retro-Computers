...
#ifdef PMMU
long     Ppid    = cpid(0);	/* PMMU coprocessor ID */
#endif
...
long     Fpid    = cpid(1);	/* floating point coprocessor ID */
...
struct ranges {
	long     r_min;  /* minimum value allowed */
	long     r_max;  /* max. allowed */
	long     r_mask; /* return value mask */
	};
...
/* op2   second opcode (e.g. F-lines) */
{
	...
	long tmp;
	long dist;      /* distance on branches */
	long rlist;     /* bit map of register list */
	char    *ptmp;
	int  i, len;
	...
	case QUKEA:             /* quick immediate to ea */
		...
		eword( op + (check(ea1->const1,QUK)<<9) + size76() + modreg(ea2) );
		finish(ea2);
		break;

	case IMMB:              /* immediate byte */
		eword(op);
		eword(check(ea1->const1,UBYTE));
		break;

	case IMMW:              /* immediate word */
		eword(op);
		eword(check(ea1->const1,XWORD));
		break;

	case IMMWS:             /* immediate word, signed */
		eword(op);
		eword(check(ea1->const1,SWORD));
		break;

	case IMM3:              /* immediate value, 3 bits */
		eword( op + check(ea1->const1,LOW3) );
		break;

	case IMM4:              /* immediate value, 4 bits */
		eword( op + check(ea1->const1,LOW4) );
		break;
	...
	case QSHIFT:            /* immediate (quick fmt) shift */
		eword( op + (check(ea1->const1,QUK)<<9) + size76() + ea2->reg);
		break;
	...
	case MOVEQ:             /* move quick */
		eword(op + (ea2->reg<<9) + check(ea1->const1,SBYTE) );
		break;
	...
	case FMOVE:             /* floating move */
		...
		if( ea3->type==EMPTY )
			...
			eword( op2 + (ea1->reg<<7) + check(ea3->const1,LOW7S) );
		else
	...
	case FMOVECR:           /* floating move constant rom */
		eword(op + Fpid);
		eword( op2 + (ea2->reg<<7) + check(ea1->const1,LOW7) );
		break;
	...
	case PFLUSH:            /* PFLUSH, PFLUSHG */
		if( ea3->type != EMPTY )
			op += modreg(ea3);
		eword( op + Ppid );
		eword( op2 + (check(ea2->const1,LOW4)<<5) + pmmu_fc(ea1) );
		if(ea3->type != EMPTY)
			finish(ea3);
		break;
	...
	case PTEST:             /* PMMU test logical address */
		...
		if( ea4->type == AN )
			op2 += (ea4->reg+8)<<5;
		eword( op2 + (check(ea3->const1,LOW3)<<10) + pmmu_fc(ea1) );
		finish(ea2);
		break;
	...
	case CPINH:             /* co-processor inherent */
		eword( op + cpid(ea1->const1) );
		eword( check(ea2->const1,LOW6) );
		break;

	case CPBCC:             /* co-processor branch on condition */
		eword( op + cpid(ea1->const1)
			  + (Size==L ? 0x40 : 0)
			  + check(ea2->const1,LOW6) );
		dist = ea3->const1 - Pc;
		if( Size == L )
			elong(dist);
		else
	...
	case CPDBCC:            /* co-processor dec & branch */
		eword( op + cpid(ea1->const1) + ea2->reg);
		eword( check(ea2->const1,LOW6) );
		dist = ea3->const1 - Pc;
		eword(check(dist,SWORD));
		break;
	...
	case CPGEN:             /* co-processor generic */
		eword(op + cpid(ea1->const1) + modreg(ea3));
		eword( ea2->const1 );
		finish(ea3);
		break;

	case CPEA:              /* co-processor, 1 ea (save/restore) */
		eword(op + cpid(ea1->const1) + modreg(ea2));
		finish(ea2);
		break;
	...
	case CPTRAPCC:          /* co-processor trap on condition */
		eword(op + cpid(ea1->const1) + (Size==W ? 2 : 3 ));
		eword( check(ea2->const1,LOW6) );
		if( Size == L )
			elong(ea3->const1);
		else
	...
	case EQU:               /* equate */
		if(*Label){
			install(Label,ea1->const1,SYM);
			Old_pc = ea1->const1;    /* override normal */
			P_force++;
			}
		else
	...
	case DC:                /* define constants */
		while(ea1->type != EMPTY )
		{
			if( ea1->type == EXPR)
			{
				switch(Size)
				{
					case B: emit(ea1->const1); break;
					case U:
					case W: eword(ea1->const1); break;
					case L: elong(ea1->const1); break;
				}
			}
			else
			{   /* must be STRING */
				ptmp = (char *)ea1->const1;
				len = strlen(ptmp);
				while(*ptmp) emit(*ptmp++);   /* output bytes */

				switch(Size)	 /* fill to word or long if required */
				{
					case B: len = 0;
					        break;
					case U:
	...
	case DS:                /* define storage */
		...
		ea1->const1 = tmp;       /* #items was zero, implies align */
	...
	case DCB:                /* define constant block */
		tmp = ea1->const1;
		switch(Size)
		{
			case B: while (tmp--) emit(ea2->const1);
			        break;
			case U:
			case W: while (tmp--) eword(ea2->const1);
		           break;
			case L: while (tmp--) elong(ea2->const1);
			        break;
		}
		break;
...
check(long const1, int type)
{
	...
	if( Pass==2 ){
		...
		if( const1<lo || const1>hi ){
			sprintf(cbuf,"Value not in range: [%d,%d]",lo,hi);
			warn(cbuf);
			}
		}
	return(const1&ckrange[type].r_mask);
}
...
finish(struct ea *e)
{
	...
	case INDEX:
		switch(e->itype){
		case D16AN:
			eword(check(e->const1,SWORD));
			break;
		case BRIEF:
			eword( genxreg(e) + check(e->const1,SBYTE));
			break;
		case FULL:
	...
	case IMMED:
		if( Size == L )
			...
			eword(e->const1);        /* note: no range check here */
		break;
	case EXPR:
		if( e->siz==L )
			elong(e->const1);
