...
ACIAStat	:= ACIA+0
ACIAData	:= ACIA+1
LED			:= $C000	; WRITE TO ROM SPACE TO ACCESS LED
...
Reset:
	...
	LDX     #$FF	; Setup stack pointer to top of page 1
	TXS
BEGIN_RAM_TEST:
; TEST FIRST LOCATION
	LDA	#$55
...
; JUMP TO OK
	JMP	RAMOK
; RAMFAIL - TURN ON THE LED IF THE RAM FAILS AND STOP
RAMFAIL:
	LDA	#1
	STA LED
RAMFAIL1:
...
LOOP1:
	JSR INPUTSTR
	LDA	INSTRBUFFER			; GET FIRST LOCATION IN INPUT LINE
CHKA:
	CMP	#'I'
	BNE	SKIPA
	JSR INSTRTST
...
SKIPC:
	...
	CALLPASS16 PRINTSTR, SetClrLEDStrMsg
	JMP ENDLOOP
SKIPD:
	CMP	#'L'
	BNE SKIPE
	LDA INSTRBUFFER+1	; 2ND LOCATION IN INPUT LINE
	STA LED
	CALLPASS16 PRINTSTR, SetClrLEDStrMsg
	JMP ENDLOOP
SKIPE:
...
HelpMsg:
	.byte	"I-TEST INPUT STRING, D-DUMP HEX, L-LED Write, ?-DETAILED HELP",$0D,$0A,$00
InputStrMsg:
...
TestDumpStrMsg:
	.byte " D XX-Dump Memory Block (Ex: DA0 dumps block $DA-$DAFF",$0D,$0A,$00
SetClrLEDStrMsg:
	.byte " L#-LED Write: 0= Off, 1=On",$0D,$0A,$00
.segment "VECTS"
.org $FFFA
	.word	Reset		; NMI 
	.word	Reset		; RESET 
