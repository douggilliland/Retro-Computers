;
; Enhanced BASIC for the C02 Pocket SBC, Version 2.22p4C
; Original version/source by Lee Davison w/updates to Version 2.22
; Patches and updates by Klaus Dorman Version 2.22p4
...
LAB_23C5
      ...
      LDA   (ut2_pl)          ; get string end +1 byte
      PHA                     ; push it
      LDA   #$00              ; clear A
      STA   (ut2_pl)          ; terminate string with $00
      JSR   LAB_GBYT          ; scan memory
      JSR   LAB_2887          ; get FAC1 from string
      PLA                     ; restore string end +1 byte
      STA   (ut2_pl)          ; put string end byte back
...
; then find the end of program (two $00), using Itemp/h,
...
; Basic code. The end is marked by two $00 bytes.
;
; This routine, finds the next line of Basic code updates (Itempl)
; and continues to the end of program code and increments (Itempl)
; by two, which then points to the start of free memory.
...
upd_bas
     ...
     BRA     scan_bas         ; loop back to find next line

chk_next
     LDA     (Itempl),y       ; get next byte
...
inc_line
     ...
     INC     Itemph           ; increment low byte count
skp_ith
