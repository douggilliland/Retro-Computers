...
Your assembly program can receive a 16 bit parameter passed in from the function by calling `DEINT` at `0x0AF8`. The parameter is stored in register pair `DE`.

When your assembly program is finished it can return a 16 bit parameter stored in `A` (MSB) and `B` (LSB) by jumping to `ABPASS` which is located at `0x1273`.
...
DEINT           .EQU    $0AF8   ; Function DEINT to get USR(x) into DE registers
ABPASS          .EQU    $1273   ; Function ABPASS to put output into AB register for return
