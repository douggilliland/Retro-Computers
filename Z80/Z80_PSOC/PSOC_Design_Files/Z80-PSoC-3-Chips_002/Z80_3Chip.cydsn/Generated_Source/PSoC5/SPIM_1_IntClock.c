...
void SPIM_1_IntClock_Start(void) 
{
    ...
    SPIM_1_IntClock_CLKEN |= SPIM_1_IntClock_CLKEN_MASK;
	SPIM_1_IntClock_CLKSTBY |= SPIM_1_IntClock_CLKSTBY_MASK;
}
...
void SPIM_1_IntClock_Stop(void) 
{
    ...
    SPIM_1_IntClock_CLKEN &= (uint8)(~SPIM_1_IntClock_CLKEN_MASK);
	SPIM_1_IntClock_CLKSTBY &= (uint8)(~SPIM_1_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SPIM_1_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SPIM_1_IntClock_CLKEN & SPIM_1_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SPIM_1_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SPIM_1_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SPIM_1_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SPIM_1_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SPIM_1_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SPIM_1_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPIM_1_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SPIM_1_IntClock_GetSourceRegister();
    uint16 oldDivider = SPIM_1_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SPIM_1_IntClock_CLKEN & SPIM_1_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SPIM_1_IntClock_DIV_PTR, clkDivider);
                SPIM_1_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SPIM_1_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SPIM_1_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SPIM_1_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(SPIM_1_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: SPIM_1_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SPIM_1_IntClock_MOD_SRC |= modeBitMask & (uint8)SPIM_1_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: SPIM_1_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SPIM_1_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SPIM_1_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: SPIM_1_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SPIM_1_IntClock_MOD_SRC & (uint8)(SPIM_1_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: SPIM_1_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SPIM_1_IntClock_GetDividerRegister();
    uint8 oldSrc = SPIM_1_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SPIM_1_IntClock_MOD_SRC |= CYCLK_SSS;
        SPIM_1_IntClock_MOD_SRC =
            (SPIM_1_IntClock_MOD_SRC & (uint8)(~SPIM_1_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SPIM_1_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SPIM_1_IntClock_MOD_SRC & SPIM_1_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SPIM_1_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SPIM_1_IntClock_PHASE = clkPhase & SPIM_1_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SPIM_1_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SPIM_1_IntClock_PHASE & SPIM_1_IntClock_PHASE_MASK;
}
