...
/*******************************************************************************
* Function Name: MAIN_CLK_1_Start
********************************************************************************
*
* Summary:
...
void MAIN_CLK_1_Start(void) 
{
    ...
    MAIN_CLK_1_CLKEN |= MAIN_CLK_1_CLKEN_MASK;
	MAIN_CLK_1_CLKSTBY |= MAIN_CLK_1_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: MAIN_CLK_1_Stop
********************************************************************************
*
* Summary:
...
void MAIN_CLK_1_Stop(void) 
{
    ...
    MAIN_CLK_1_CLKEN &= (uint8)(~MAIN_CLK_1_CLKEN_MASK);
	MAIN_CLK_1_CLKSTBY &= (uint8)(~MAIN_CLK_1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: MAIN_CLK_1_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((MAIN_CLK_1_CLKEN & MAIN_CLK_1_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = MAIN_CLK_1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = MAIN_CLK_1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* MAIN_CLK_1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(MAIN_CLK_1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(MAIN_CLK_1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: MAIN_CLK_1_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: MAIN_CLK_1_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = MAIN_CLK_1_GetSourceRegister();
    uint16 oldDivider = MAIN_CLK_1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = MAIN_CLK_1_CLKEN & MAIN_CLK_1_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((MAIN_CLK_1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: MAIN_CLK_1_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(MAIN_CLK_1_DIV_PTR);
}


/*******************************************************************************
* Function Name: MAIN_CLK_1_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    MAIN_CLK_1_MOD_SRC |= modeBitMask & (uint8)MAIN_CLK_1_MODE_MASK;
}


/*******************************************************************************
* Function Name: MAIN_CLK_1_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    MAIN_CLK_1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(MAIN_CLK_1_MODE_MASK));
}


/*******************************************************************************
* Function Name: MAIN_CLK_1_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return MAIN_CLK_1_MOD_SRC & (uint8)(MAIN_CLK_1_MODE_MASK);
}


/*******************************************************************************
* Function Name: MAIN_CLK_1_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = MAIN_CLK_1_GetDividerRegister();
    uint8 oldSrc = MAIN_CLK_1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        MAIN_CLK_1_MOD_SRC |= CYCLK_SSS;
        MAIN_CLK_1_MOD_SRC =
            (MAIN_CLK_1_MOD_SRC & (uint8)(~MAIN_CLK_1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: MAIN_CLK_1_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return MAIN_CLK_1_MOD_SRC & MAIN_CLK_1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: MAIN_CLK_1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    MAIN_CLK_1_PHASE = clkPhase & MAIN_CLK_1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: MAIN_CLK_1_GetPhase
********************************************************************************
*
* Summary:
...
{
    return MAIN_CLK_1_PHASE & MAIN_CLK_1_PHASE_MASK;
}
