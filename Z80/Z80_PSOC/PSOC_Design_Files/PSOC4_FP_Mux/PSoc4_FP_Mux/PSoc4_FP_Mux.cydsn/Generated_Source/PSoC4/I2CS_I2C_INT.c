...
CY_ISR(I2CS_I2C_ISR)
{
...
#ifdef I2CS_I2C_ISR_ENTRY_CALLBACK
    I2CS_I2C_ISR_EntryCallback();
#endif /* I2CS_I2C_ISR_ENTRY_CALLBACK */

#if (I2CS_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = I2CS_I2C_ACK_ADDR;
    ...
    if(I2CS_CHECK_INTR_I2C_EC_MASKED(I2CS_INTR_I2C_EC_WAKE_UP))
    {
    ...
    */
    if(I2CS_CHECK_I2C_FSM_MASTER)
    {
        #if(I2CS_I2C_MASTER)
        {
            ...
            */
            if(I2CS_CHECK_INTR_MASTER_MASKED(I2CS_INTR_MASTER_I2C_BUS_ERROR))
            {
                I2CS_mstrStatus |= (uint16) (I2CS_I2C_MSTAT_ERR_XFER |
                                                         I2CS_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = I2CS_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(I2CS_CHECK_INTR_MASTER_MASKED(I2CS_INTR_MASTER_I2C_ARB_LOST))
            {
                I2CS_mstrStatus |= (uint16) (I2CS_I2C_MSTAT_ERR_XFER |
                                                         I2CS_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = I2CS_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(I2CS_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(I2CS_CHECK_I2C_MASTER_CMD(I2CS_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    I2CS_mstrStatus |= (uint16) (I2CS_I2C_MSTAT_ERR_XFER |
                                                             I2CS_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                I2CS_mstrStatus |= (uint16) I2CS_GET_I2C_MSTAT_CMPLT;

                #if(I2CS_I2C_MULTI_MASTER_SLAVE)
                {
                    if(I2CS_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(I2CS_CHECK_I2C_MASTER_CMD(I2CS_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            I2CS_I2C_MASTER_CLEAR_START;

                            endTransfer = I2CS_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!I2CS_CHECK_INTR_SLAVE_MASKED(I2CS_INTR_SLAVE_I2C_BUS_ERROR))
                               && I2CS_CHECK_INTR_MASTER_MASKED(I2CS_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = I2CS_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            I2CS_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            I2CS_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                        ...
                        else
                        {
                            ...
                            I2CS_state = I2CS_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (I2CS_CHECK_I2C_FSM_SLAVE) */
    {
        #if(I2CS_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or I2CS_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(I2CS_CHECK_INTR_SLAVE_MASKED(I2CS_INTR_SLAVE_I2C_BUS_ERROR |
                                                        I2CS_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(I2CS_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    I2CS_slStatus &= (uint8) ~I2CS_I2C_SSTAT_RD_BUSY;
                    I2CS_slStatus |= (uint8) (I2CS_I2C_SSTAT_RD_ERR |
                                                          I2CS_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    I2CS_slStatus &= (uint8) ~I2CS_I2C_SSTAT_WR_BUSY;
                    I2CS_slStatus |= (uint8) (I2CS_I2C_SSTAT_WR_ERR |
                                                          I2CS_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(I2CS_CHECK_I2C_FSM_MASTER)
    {
        #if(I2CS_I2C_MASTER)
        {
            ...
            */
            if(I2CS_CHECK_INTR_MASTER_MASKED(I2CS_INTR_MASTER_I2C_STOP))
            {
                I2CS_ClearMasterInterruptSource(I2CS_INTR_MASTER_I2C_STOP);
            ...
            {
                if(I2CS_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(I2CS_CHECK_INTR_MASTER_MASKED(I2CS_INTR_MASTER_I2C_NACK))
                    {
                        I2CS_ClearMasterInterruptSource(I2CS_INTR_MASTER_I2C_NACK);

                        I2CS_mstrStatus |= (uint16) (I2CS_I2C_MSTAT_ERR_XFER |
                                                                 I2CS_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(I2CS_CHECK_I2C_FSM_RD) /* Reading */
                        {
                            I2CS_state = I2CS_I2C_FSM_MSTR_RD_DATA;
                        }
                        else /* Writing */
                        {
                            I2CS_state = I2CS_I2C_FSM_MSTR_WR_DATA;
                            if(0u != I2CS_mstrWrBufSize)
                            {
                ...
                if(I2CS_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(I2CS_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(I2CS_CHECK_INTR_RX_MASKED(I2CS_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  I2CS_mstrRdBufSize -
                                        (I2CS_mstrRdBufIndex + I2CS_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > I2CS_I2C_FIFO_SIZE)
                            {
                                diffCount = I2CS_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    I2CS_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                I2CS_mstrRdBufPtr[I2CS_mstrRdBufIndex] = (uint8)
                                                                                        I2CS_RX_FIFO_RD_REG;
                                I2CS_mstrRdBufIndex++;
                            }
                        }
                        ...
                        */
                        else if(I2CS_CHECK_INTR_RX_MASKED(I2CS_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            I2CS_mstrRdBufPtr[I2CS_mstrRdBufIndex] = (uint8) I2CS_RX_FIFO_RD_REG;
                            I2CS_mstrRdBufIndex++;

                            if(I2CS_mstrRdBufIndex < I2CS_mstrRdBufSize)
                            {
                                I2CS_I2C_MASTER_GENERATE_ACK;
                            }
                            else
                            {
                               endTransfer = I2CS_I2C_CMPLT_ANY_TRANSFER;
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        if(I2CS_CHECK_INTR_MASTER_MASKED(I2CS_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            I2CS_mstrWrBufIndexTmp -= (I2CS_GET_TX_FIFO_ENTRIES +
                                                                   I2CS_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            I2CS_mstrWrBufIndex = I2CS_mstrWrBufIndexTmp;

                            I2CS_mstrStatus |= (uint16) (I2CS_I2C_MSTAT_ERR_XFER |
                                                                     I2CS_I2C_MSTAT_ERR_SHORT_XFER);
                        ...
                        */
                        else if(I2CS_CHECK_INTR_TX_MASKED(I2CS_INTR_TX_EMPTY))
                        {
                            while(I2CS_I2C_FIFO_SIZE != I2CS_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(I2CS_mstrWrBufIndexTmp < I2CS_mstrWrBufSize)
                                {
                                #if(!I2CS_CY_SCBIP_V0)
                                    ...
                                    */
                                    if(I2CS_mstrWrBufIndexTmp == (I2CS_mstrWrBufSize - 1u))
                                    {
                                        I2CS_ClearTxInterruptSource(I2CS_INTR_TX_UNDERFLOW);
                                        I2CS_SetTxInterruptMode(I2CS_INTR_TX_UNDERFLOW);
                                    }
                                 #endif /* (!I2CS_CY_SCBIP_V0) */

                                    /* Put data into TX FIFO */
                                    I2CS_TX_FIFO_WR_REG = (uint32) I2CS_mstrWrBufPtr[I2CS_mstrWrBufIndexTmp];
                                    I2CS_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                        ...
                        */
                        else if(I2CS_CHECK_INTR_TX_MASKED(I2CS_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            I2CS_mstrWrBufIndex = I2CS_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(I2CS_CHECK_I2C_MODE_NO_STOP(I2CS_mstrControl))
                    {
                        /* On-going transaction is suspended: the ReStart is generated by the API request */
                        I2CS_mstrStatus |= (uint16) (I2CS_I2C_MSTAT_XFER_HALT |
                                                                 I2CS_GET_I2C_MSTAT_CMPLT);

                        I2CS_state = I2CS_I2C_FSM_MSTR_HALT;
                    }
                    else
                    {
    ...
    } /* (I2CS_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(I2CS_CHECK_I2C_FSM_SLAVE)
    {
        #if(I2CS_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(I2CS_CHECK_INTR_SLAVE_MASKED(I2CS_INTR_SLAVE_I2C_NACK))
            {
                I2CS_ClearSlaveInterruptSource(I2CS_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (I2CS_GET_TX_FIFO_ENTRIES + I2CS_GET_TX_FIFO_SR_VALID);

                if(I2CS_slOverFlowCount > diffCount) /* Overflow */
                {
                    I2CS_slStatus |= (uint8) I2CS_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    I2CS_slRdBufIndexTmp -= (diffCount - I2CS_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                I2CS_slRdBufIndex = I2CS_slRdBufIndexTmp;
                ...
                I2CS_slStatus &= (uint8) ~I2CS_I2C_SSTAT_RD_BUSY;
                I2CS_slStatus |= (uint8)  I2CS_I2C_SSTAT_RD_CMPLT;
                I2CS_state     =  I2CS_I2C_FSM_IDLE;
                
            #ifdef I2CS_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                I2CS_I2C_SlaveCompleteCallback();
            #endif /* I2CS_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(I2CS_CHECK_INTR_SLAVE_MASKED(I2CS_INTR_SLAVE_I2C_WRITE_STOP))
            {
                I2CS_ClearSlaveInterruptSource(I2CS_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (I2CS_I2C_CTRL_REG & I2CS_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != I2CS_GET_RX_FIFO_ENTRIES)
                    {
                        #if(I2CS_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == I2CS_GET_RX_FIFO_ENTRIES) &&
                               (I2CS_CHECK_INTR_SLAVE_MASKED(I2CS_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        I2CS_slWrBufPtr[I2CS_slWrBufIndex] = (uint8) I2CS_RX_FIFO_RD_REG;
                        I2CS_slWrBufIndex++;
                    }
                ...
                if(I2CS_CHECK_INTR_RX(I2CS_INTR_RX_OVERFLOW))
                {
                    I2CS_slStatus |= (uint8) I2CS_I2C_SSTAT_WR_OVFL;
                }
                ...
                I2CS_slStatus &= (uint8) ~I2CS_I2C_SSTAT_WR_BUSY;
                I2CS_slStatus |= (uint8)  I2CS_I2C_SSTAT_WR_CMPLT;
                I2CS_state     =  I2CS_I2C_FSM_IDLE;
        ...
        #endif /* (I2CS_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
            ...
            #endif /* (I2CS_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    ...
                    /* TX direction: master reads from slave */
                    {
                        ...
                        I2CS_slRdBufIndexTmp = I2CS_slRdBufIndex;

                        /* Start master reading */
                        I2CS_slStatus |= (uint8) I2CS_I2C_SSTAT_RD_BUSY;
                        I2CS_state     = I2CS_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (I2CS_slWrBufSize - I2CS_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == I2CS_I2C_FIFO_SIZE)
                            {
                                ...
                                I2CS_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                I2CS_ENABLE_SLAVE_AUTO_DATA_ACK;
                    ...
                    I2CS_ENABLE_INTR_SLAVE(I2CS_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2CS_CY_SCBIP_V0) */
            ...
            */
            if(I2CS_CHECK_INTR_RX_MASKED(I2CS_INTR_RX_FULL))
            {
                ...
                diffCount =  I2CS_slWrBufSize -
                            (I2CS_slWrBufIndex + I2CS_I2C_FIFO_SIZE);

                if(diffCount > I2CS_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = I2CS_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
            ...
            */
            else if(I2CS_CHECK_INTR_RX_MASKED(I2CS_INTR_RX_NOT_EMPTY))
            {
                ...
                if(I2CS_slWrBufIndex < I2CS_slWrBufSize)
                {
                    ...
                    I2CS_slWrBufPtr[I2CS_slWrBufIndex] = (uint8) diffCount;
                    I2CS_slWrBufIndex++;
                }
            ...
            */
            if(I2CS_CHECK_INTR_TX_MASKED(I2CS_INTR_TX_EMPTY))
            {
                while(I2CS_I2C_FIFO_SIZE != I2CS_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(I2CS_slRdBufIndexTmp < I2CS_slRdBufSize)
                    /* Data from buffer */
                    {
                        I2CS_TX_FIFO_WR_REG = (uint32) I2CS_slRdBufPtr[I2CS_slRdBufIndexTmp];
                        I2CS_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        I2CS_TX_FIFO_WR_REG = I2CS_I2C_SLAVE_OVFL_RETURN;

                        if(I2CS_slOverFlowCount <= I2CS_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            I2CS_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef I2CS_I2C_ISR_EXIT_CALLBACK
    I2CS_I2C_ISR_ExitCallback();
#endif /* I2CS_I2C_ISR_EXIT_CALLBACK */
