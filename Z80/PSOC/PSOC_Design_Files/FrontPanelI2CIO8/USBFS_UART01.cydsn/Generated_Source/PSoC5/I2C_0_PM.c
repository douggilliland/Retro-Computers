...
I2C_0_BACKUP_STRUCT I2C_0_backup =
{
    I2C_0_DISABLE,
    ...
    I2C_0_DEFAULT_CFG,
    I2C_0_DEFAULT_ADDR,
    LO8(I2C_0_DEFAULT_DIVIDE_FACTOR),
    HI8(I2C_0_DEFAULT_DIVIDE_FACTOR),
#else  /* (I2C_0_UDB_IMPLEMENTED) */
    I2C_0_DEFAULT_CFG,
#endif /* (I2C_0_FF_IMPLEMENTED) */

#if (I2C_0_TIMEOUT_ENABLED)
    I2C_0_DEFAULT_TMOUT_PERIOD,
    I2C_0_DEFAULT_TMOUT_INTR_MASK,
#endif /* (I2C_0_TIMEOUT_ENABLED) */
};

#if ((I2C_0_FF_IMPLEMENTED) && (I2C_0_WAKEUP_ENABLED))
    volatile uint8 I2C_0_wakeupSource;
#endif /* ((I2C_0_FF_IMPLEMENTED) && (I2C_0_WAKEUP_ENABLED)) */


/*******************************************************************************
* Function Name: I2C_0_SaveConfig
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  I2C_0_backup - The global variable used to save the component
...
{
#if (I2C_0_FF_IMPLEMENTED)
    #if (I2C_0_WAKEUP_ENABLED)
        uint8 intState;
    #endif /* (I2C_0_WAKEUP_ENABLED) */

    /* Store registers before enter low power mode */
    I2C_0_backup.cfg     = I2C_0_CFG_REG;
    I2C_0_backup.xcfg    = I2C_0_XCFG_REG;
...
#if (I2C_0_WAKEUP_ENABLED)
    /* Disable master */
    I2C_0_CFG_REG &= (uint8) ~I2C_0_ENABLE_MASTER;

    /* Enable backup regulator to keep block powered in low power mode */
    intState = CyEnterCriticalSection();
    I2C_0_PWRSYS_CR1_REG |= I2C_0_PWRSYS_CR1_I2C_REG_BACKUP;
    ...
    */
    ...
    I2C_0_XCFG_REG |= I2C_0_XCFG_FORCE_NACK;
    while (0u == (I2C_0_XCFG_REG & I2C_0_XCFG_RDY_TO_SLEEP))
    {
    }

    /* Setup wakeup interrupt */
    I2C_0_DisableInt();
    (void) CyIntSetVector(I2C_0_ISR_NUMBER, &I2C_0_WAKEUP_ISR);
    I2C_0_wakeupSource = 0u;
    I2C_0_EnableInt();
#endif /* (I2C_0_WAKEUP_ENABLED) */

#else
    /* Store only address match bit */
    I2C_0_backup.control = (I2C_0_CFG_REG & I2C_0_CTRL_ANY_ADDRESS_MASK);
#endif /* (I2C_0_FF_IMPLEMENTED) */

#if (I2C_0_TIMEOUT_ENABLED)
    I2C_0_TimeoutSaveConfig();
#endif /* (I2C_0_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2C_0_Sleep
********************************************************************************
*
* Summary:
...
void I2C_0_Sleep(void) 
{
    ...
    I2C_0_backup.enableState = I2C_0_DISABLE;
...
#else
    /* Store enable state */
    I2C_0_backup.enableState = (uint8) I2C_0_IS_ENABLED;

    if (0u != I2C_0_backup.enableState)
    {
        I2C_0_Stop();
    }
#endif /* (I2C_0_WAKEUP_ENABLED) */
...
/*******************************************************************************
* Function Name: I2C_0_RestoreConfig
********************************************************************************
*
* Summary:
*  The Enable wakeup from Sleep Mode selection influences this function
...
*
* Side Effects:
*  Calling this function before I2C_0_SaveConfig() or
*  I2C_0_Sleep() will lead to unpredictable results.
*
*******************************************************************************/
void I2C_0_RestoreConfig(void) 
{
    ...
    if (I2C_0_CHECK_PWRSYS_I2C_BACKUP)
    /* Low power mode was Sleep - backup regulator is enabled */
    {
        /* Enable backup regulator in active mode */
        intState = CyEnterCriticalSection();
        I2C_0_PWRSYS_CR1_REG &= (uint8) ~I2C_0_PWRSYS_CR1_I2C_REG_BACKUP;
        CyExitCriticalSection(intState);

        /* Restore master */
        I2C_0_CFG_REG = I2C_0_backup.cfg;
    }
    else
    /* Low power mode was Hibernate - backup regulator is disabled. All registers are cleared */
    {
        ...
        I2C_0_backup.enableState = I2C_0_ENABLE;
    #endif /* (I2C_0_WAKEUP_ENABLED) */

        /* Restore component registers after Hibernate */
        I2C_0_XCFG_REG    = I2C_0_backup.xcfg;
        I2C_0_CFG_REG     = I2C_0_backup.cfg;
...
#else
    I2C_0_CFG_REG = I2C_0_backup.control;
#endif /* (I2C_0_FF_IMPLEMENTED) */

#if (I2C_0_TIMEOUT_ENABLED)
    I2C_0_TimeoutRestoreConfig();
#endif /* (I2C_0_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2C_0_Wakeup
********************************************************************************
*
* Summary:
