...
{    
    SPIM_ClearTxBuffer();
    SPIM_PutArray(buff,bc);         // !!! PutArray => max uint8 => 255
    while (!(SPIM_ReadTxStatus()&SPIM_STS_SPI_IDLE)) {;}     // SPI_IDLE    
}
...
static void rcvr_mmc(BYTE *buff, UINT bc)
{
    ...
    do
    {
        ...
        while (!(SPIM_ReadRxStatus()&SPIM_STS_RX_FIFO_NOT_EMPTY)) {;}    // GOT one byte
        *buff++ = (BYTE)SPIM_ReadRxData();
    } while (--bc);   
}
...
static int xmit_datablock(const BYTE *buff, BYTE token)	/* 1:OK, 0:Failed */
{
	...
	if (token != 0xFD) {		/* Is it data token? */
		xmit_mmc(buff+  0, 128);	/* Xmit the 512 byte data block to MMC */
		xmit_mmc(buff+128, 128);
		xmit_mmc(buff+256, 128);
		xmit_mmc(buff+384, 128);
...
DSTATUS disk_initialize(BYTE drv) /* Physical drive nmuber (0) */
{
    ...
    sprintf(str,"SPI div : %d\n\r",SPIM_IntClock_GetDividerRegister());
    UART_PutString(str);
    
    // SLOW CLOCK
    SPIM_IntClock_SetDivider(59);
    ...
    if (ty) {    SPIM_IntClock_SetDivider(2);} // FAST CLOCK
