...
?boot:	jmp boot	; 1 initial entry on cold start
?wboot:	jmp wboot	; 2 reentry on program exit, warm start

?const:	jmp const	; 3 return console input status
?conin:	jmp conin	; 4 return console input character
?cono:	jmp conout	; 5 send console output character
?list:	jmp list	; 6 send list output character
?auxo:	jmp auxout	; 7 send auxilliary output character
?auxi:	jmp auxin	; 8 return auxilliary input character

?home:	jmp home	; 9 set disks to logical home
?sldsk:	jmp seldsk	; 10 select disk drive, return disk parameter info
?sttrk:	jmp settrk	; 11 set disk track
?stsec:	jmp setsec	; 12 set disk sector
?stdma:	jmp setdma	; 13 set disk I/O memory address
?read:	jmp read	; 14 read physical block(s)
?write:	jmp write	; 15 write physical block(s)

?lists:	jmp listst	; 16 return list device status
?sctrn:	jmp sectrn	; 17 translate logical to physical sector

?conos:	jmp conost	; 18 return console output status
?auxis:	jmp auxist	; 19 return aux input status
?auxos:	jmp auxost	; 20 return aux output status
?dvtbl:	jmp devtbl	; 21 return address of device def table
?devin:	jmp ?cinit	; 22 change baud rate of device

?drtbl:	jmp getdrv	; 23 return address of disk drive table
?mltio:	jmp multio	; 24 set multiple record count for disk I/O
?flush:	jmp flush	; 25 flush BIOS maintained disk caching

?mov:	jmp ?move	; 26 block move memory to memory
?tim:	jmp ?time	; 27 Signal Time and Date operation
?bnksl:	jmp bnksel	; 28 select bank for code execution and default DMA
?stbnk:	jmp setbnk	; 29 select different bank for disk I/O DMA operations.
?xmov:	jmp ?xmove	; 30 set source and destination banks for one operation

	jmp dmount	; 31 mount volumes
	jmp 0		; 32 reserved for future expansion
	jmp 0		; 33 reserved for future expansion
...
boot$1:
	call set$jumps				; we're in bank 1 now....
