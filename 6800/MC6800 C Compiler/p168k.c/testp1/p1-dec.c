...
extern short nxtarg, argoff, datareg, addrreg, flptreg;
extern char optE;
extern char optQ;
...
#ifdef MC68020
        if (optQ && (strofs & 2) && (dtype >= INT) && (!isarch(dtype))) {
          strofs += 2;
          symloc->sflags |= FALND;
        }
#endif
  ...
  if (!size) {
    switch(usize) {
      ...
      case STRUCT:
        ...
        size = (unsigned short)memptr->sstore + sizeit(memptr);
#ifdef ALIGN
        if (size & 1)
          size++;
#endif
  ...
  if ((tp & 0x30) == (PTR << 4)) {
    ...
    if (tp < FLOAT) {
      ...
      if (tp <= DUBLE) {
        if (flptreg < NUMFREG) {
          sym->sclass = REG;
          dtype = sym->stype = DUBLE;
          if (flag) {
            sym->sblklv = 64 + flptreg++;
          }
          else {
            sym->sstore = 64 + flptreg++;
            outregdf(sym);
          }
          return(TRUE);
        }
...
/* rotate dimension list for typedefs */

rotdims() {
  register int *p1, *p2, *p3;
  register short ac, tac, typ, temp;
  ...
  typ = (typdfsym->stype >> 4);
  for (tac=0; (typ & 0x03) == ARAY; tac++)
    typ >>= 2;
  if (tac == ac)
    return;
  while (tac) {
