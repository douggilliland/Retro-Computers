...
char opalong;                  /* doing opasn on long */
...
int reguse;                    /* registers used in function */
...
char namebuf[10];              /* temp buffer for building names */
...
int lretlab;                   /* bss space label for long functions */
char notdone;                  /* not finished with address flag */
...
initial() {
  ...
  extra = 0;
  extflg = 0;
  if ((fout = creat(oname, 3)) == ERROR) {
    perror("Can't create assembler file - pass2 aborted.\n");
    exit(255);
  }
  nxtlab = 10000;
}
