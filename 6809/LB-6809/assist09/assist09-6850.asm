...
XAVTBL	EQU		0				; ADDRESS OF VECTOR TABLE
XCMDL1	EQU     2               ; FIRST COMMAND LIST
XRSVD   EQU     4               ; RESERVED HARDWARE VECTOR
XSWI3   EQU     6               ; SWI3 ROUTINE
XSWI2   EQU     8               ; SWI2 ROUTINE
XFIRQ   EQU     10              ; FIRQ ROUTINE
XIRQ    EQU     12              ; IRQ ROUTINE
XSWI    EQU     14              ; SWI ROUTINE
XNMI    EQU     16              ; NMI ROUTINE
XRESET  EQU     18              ; RESET ROUTINE
XCION   EQU     20              ; CONSOLE ON
XCIDTA  EQU     22              ; CONSOLE INPUT DATA
XCIOFF  EQU     24              ; CONSOLE INPUT OFF
XCOON   EQU     26              ; CONSOLE OUTPUT ON
XCODTA  EQU     28              ; CONSOLE OUTPUT DATA
XCOOFF  EQU     30              ; CONSOLE OUTPUT OFF
XHSDTA  EQU     32              ; HIGH SPEED PRINTDATA
XBSON   EQU     34              ; PUNCH/LOAD ON
XBSDTA EQU     	36              ; PUNCH/LOAD DATA
XBSOFF  EQU     38              ; PUNCH/LOAD OFF
XPAUSE  EQU     40              ; TASK PAUSE ROUTINE
XEXPAN  EQU     42              ; EXPRESSION ANALYZER
XCMDL2  EQU     44              ; SECOND COMMAND LIST
XACIA   EQU     46              ; ACIA ADDRESS
XPAD    EQU     48              ; CHARACTER PAD AND NEW LINE PAD
XECHO   EQU     50              ; ECHO/LOAD AND NULL BKPT FLAG
XPTM    EQU     52              ; PTM ADDRESS
...
ZMONTR  STS     <RSTACK         ; SAVE FOR BAD STACK RECOVERY
        ...
        JSR     [VECTAB+XCION,PCR] ; READY CONSOLE INPUT
        JSR     [VECTAB+XCOON,PCR] ; READY CONSOLE OUTPUT
        LEAX    SIGNON,PCR         ; READY SIGNON EYE-CATCHER
        SWI                     ; PERFORM
        FCB     PDATA           ; PRINT STRING
ZMONT2  LDX     <VECTAB+XPTM    ; LOAD PTM ADDRESS
...
CMDGOT  SUBA   #CR              ; SET ZERO IF CARRIAGE RETURN
        STA    -3,U             ; SETUP FLAG
        LDX    <VECTAB+XCMDL1   ; START WITH FIRST CMD LIST
CMDSCH  LDB    ,X+              ; LOAD ENTRY LENGTH
        BPL    CMDSME           ; BRANCH IF NOT LIST END
        LDX    <VECTAB+XCMDL2   ; NOW TO SECOND CMD LITS
...
ZOUTHX  ADDA    #$90            ; PREPARE A-F ADJUST
        ...
        DAA                     ; ADJUST
SEND    JMP     [VECTAB+XCODTA,PCR] ; SEND TO OUT HANDLER
...
ZVSWTH  LDA     1,S             ; LOAD REQUESTERS A
        ...
        LDY     <VECTAB+XAVTBL  ; LOAD VECTOR TABLE ADDRESS
        LDU     A,Y             ; U=OLD ENTRY
...
ZIN2    TST     <VECTAB+XECHO   ; ? ECHO DESIRED
...
XQPAUS  JMP   [VECTAB+XPAUSE,PCR] ; TO PAUSE ROUTINE
XQCIDT  JSR   [VECTAB+XCIDTA,PCR] ; TO INPUT ROUTINE
...
CIDTA   LDU     <VECTAB+XACIA   ; LOAD ACIA ADDRESS
...
COON   LDA      #$13            ; RESET ACIA CODE
       LDX      <VECTAB+XACIA   ; LOAD ACIA ADDRESS
...
CODTA   PSHS    U,D,CC          ; SAVE REGISTERS,WORK BYTE
        LDU     <VECTAB+XACIA   ; ADDRESS ACIA
        BSR     CODTAO          ; CALL OUTPUT CHAR SUBROUTINE
        CMPA    #DLE            ; ? DATA LINE ESCAPE
        BEQ     CODTRT          ; YES, RETURN
        LDB     <VECTAB+XPAD    ; DEFAULT TO CHAR PAD COUNT
        CMPA    #CR             ; ? CR
        BNE     CODTPD          ; BRANCH NO
        LDB     <VECTAB+XPAD+1  ; LOAD NEW LINE PAD COUNT
...
BSDPUN  LDU     <VECTAB+XPAD    ; LOAD PADDING VALUES
        LDX     4,S             ; X=FROM ADDRESS
        PSHS    U,X,D           ; CREATE STACK WORK AREA
        LDD     #24             ; SET A=0, B=24
        STB     <VECTAB+XPAD    ; SETUP 24 CHARACTER PADS
        SWI                     ; SEND NULLS OUT
        FCB     OUTCH           ; FUNCTION
        LDB     #4              ; SETUP NEW LINE PAD TO 4
        STD     <VECTAB+XPAD    ; SETUP PUNCH PADDING
* CALCULATE SIZE
...
BLDNUM  LDA     #'              ; ALLOW LEADING BLANKS
        ...
        JMP     [VECTAB+XEXPAN,PCR]   ; TO EXP ANALYZER
...
CDCNT   JSR     [VECTAB+XHSDTA,PCR] ; CALL PRINT ROUTINE
...
CCALBS  JSR     [VECTAB+XBSON,PCR] ; INITIALIZE HANDLER
        JSR     [VECTAB+XBSDTA,PCR] ; PERFORM FUNCTION
        PSHS    CC              ; SAVE RETURN CODE
        JSR     [VECTAB+XBSOFF,PCR] ; TURN OFF HANDLER
...
CTRCE3  LDU     [10,S]          ; LOAD OPCODE TO EXECUTE
        ...
        LDU     <VECTAB+XPTM    ; LOAD PTM ADDRESS
...
****************************************************
RSRVD   JMP     [VECTAB+XRSVD,PCR]      ; RESERVED VECTOR
SWI3    JMP     [VECTAB+XSWI3,PCR]      ; SWI3 VECTOR
SWI2    JMP     [VECTAB+XSWI2,PCR]      ; SWI2 VECTOR
FIRQ    JMP     [VECTAB+XFIRQ,PCR]      ; FIRQ VECTOR
IRQ     JMP     [VECTAB+XIRQ,PCR]       ; IRQ VECTOR
SWI     JMP     [VECTAB+XSWI,PCR]       ; SWI VECTOR
NMI     JMP     [VECTAB+XNMI,PCR]       ; NMI VECTOR
